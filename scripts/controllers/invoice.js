...
angular
  .module('urbanApp')
  .controller('invoiceCtrl', ['$scope', '$location', '$http', '$state', 'UserService', 'DTOptionsBuilder', '$rootScope', '$timeout',  invoiceCtrl]);
	function invoiceCtrl($scope, $location, $http, $state, UserService, DTOptionsBuilder, $rootScope, $timeout) {
  ...
  $scope.filterTransactions = function(obj = document.getElementById("fltrBtn")){
    ...
    var  url= UserService.apiRoot+'hmo/get/invoice/'+fromDate+'/'+toDate+'/'+organization+'/-';
  ...
  $scope.sendInvoice = function(ev){
    var  url= UserService.apiRoot+'hmo/send/invoice';
    ev = ev.currentTarget;
    ...
    var datum = {
      "data":{
          'emails': emails,
          'message': message,
          'invoiceID': $scope.activeInvoice.invoiceID,
          'username': sessionStorage.getItem('username'),
          'publicKey': sessionStorage.getItem('publicKey'),
          'hmoID': sessionStorage.getItem('HMOID')            
      }
  ...
  $scope.updateStatus = function(ev, x){
      if(confirm("Please confirm, Are you sure you want to update the status of the Invoice to paid?")){
        var  url= UserService.apiRoot+'hmo/update/invoice/to/1';
        ev = ev.currentTarget;
        var datum = {
          "data":{
